...
Fiber_color_Ca               <- scan("Fiber_color_Ca.5pct.genes               ", what="")
Fiber_length_LnCV            <- scan("Fiber_length_LnCV.5pct.genes            ", what="")
Fiber_quality_NS             <- scan("Fiber_quality_NS.5pct.genes             ", what="")
Flower_SD                    <- scan("Flower_SD.5pct.genes                    ", what="")
Phenology_FBFF               <- scan("Phenology_FBFF.5pct.genes               ", what="")
Plant_architecture_PHFB1     <- scan("Plant_architecture_PHFB1.5pct.genes     ", what="")
Seed_FSW                     <- scan("Seed_FSW.5pct.genes                     ", what="")
Fiber_color.category         <- scan("Fiber_color.category.5pct.genes         ", what="")
Fiber_length_Ln              <- scan("Fiber_length_Ln.5pct.genes              ", what="")
Fiber_quality_SFCn           <- scan("Fiber_quality_SFCn.5pct.genes           ", what="")
Fruiting_habit.category      <- scan("Fruiting_habit.category.5pct.genes      ", what="")
Phenology_GB                 <- scan("Phenology_GB.5pct.genes                 ", what="")
Plant_architecture_PHFB2     <- scan("Plant_architecture_PHFB2.5pct.genes     ", what="")
Seed_SCW                     <- scan("Seed_SCW.5pct.genes                     ", what="")
Fiber_color_Cb               <- scan("Fiber_color_Cb.5pct.genes               ", what="")
Fiber_length_Lw              <- scan("Fiber_length_Lw.5pct.genes              ", what="")
Fiber_quality_TrS            <- scan("Fiber_quality_TrS.5pct.genes            ", what="")
Fruiting_habit_NF            <- scan("Fruiting_habit_NF.5pct.genes            ", what="")
Phenology_TNFF               <- scan("Phenology_TNFF.5pct.genes               ", what="")
Plant_architecture_PH        <- scan("Plant_architecture_PH.5pct.genes        ", what="")
Seed_SW                      <- scan("Seed_SW.5pct.genes                      ", what="")
Fiber_color_CL               <- scan("Fiber_color_CL.5pct.genes               ", what="")
Fiber_length_UQLw            <- scan("Fiber_length_UQLw.5pct.genes            ", what="")
Flower.category              <- scan("Flower.category.5pct.genes              ", what="")
Fruiting_habit_PHTN          <- scan("Fruiting_habit_PHTN.5pct.genes          ", what="")
Plant_architecture_BA        <- scan("Plant_architecture_BA.5pct.genes        ", what="")
Plant_architecture_SP        <- scan("Plant_architecture_SP.5pct.genes        ", what="")
Fiber_length.category        <- scan("Fiber_length.category.5pct.genes        ", what="")
Fiber_quality.category       <- scan("Fiber_quality.category.5pct.genes       ", what="")
Flower_CS                    <- scan("Flower_CS.5pct.genes                    ", what="")
Fruiting_habit_TNFB          <- scan("Fruiting_habit_TNFB.5pct.genes          ", what="")
Plant_architecture.category  <- scan("Plant_architecture.category.5pct.genes  ", what="")
Seed_AL                      <- scan("Seed_AL.5pct.genes                      ", what="")
Fiber_length_L25n            <- scan("Fiber_length_L25n.5pct.genes            ", what="")
Fiber_quality_Fine           <- scan("Fiber_quality_Fine.5pct.genes           ", what="")
Flower_PC                    <- scan("Flower_PC.5pct.genes                    ", what="")
Fruiting_habit_TN            <- scan("Fruiting_habit_TN.5pct.genes            ", what="")
Plant_architecture_FB1       <- scan("Plant_architecture_FB1.5pct.genes       ", what="")
Seed_BW                      <- scan("Seed_BW.5pct.genes                      ", what="")
Fiber_length_L5n             <- scan("Fiber_length_L5n.5pct.genes             ", what="")
Fiber_quality_MR             <- scan("Fiber_quality_MR.5pct.genes             ", what="")
Flower_PS                    <- scan("Flower_PS.5pct.genes                    ", what="")
Phenology.category           <- scan("Phenology.category.5pct.genes           ", what="")
Plant_architecture_FB2       <- scan("Plant_architecture_FB2.5pct.genes       ", what="")
Seed.category                <- scan("Seed.category.5pct.genes                ", what="")
...
write.table(bothmat, file="annotated.Gohir.5pct.tbl", sep="\t", quote=F, row.names=F)
...
write.table(FCmat, file="annotated.Gohir.FCmat.5pct.tbl", sep="\t", quote=F, row.names=F)
write.table(FLmat, file="annotated.Gohir.FLmat.5pct.tbl", sep="\t", quote=F, row.names=F)
write.table(FQmat, file="annotated.Gohir.FQmat.5pct.tbl", sep="\t", quote=F, row.names=F)
write.table(Flowermat, file="annotated.Gohir.Flowermat.5pct.tbl", sep="\t", quote=F, row.names=F)
write.table(Fruitmat, file="annotated.Gohir.Fruitmat.5pct.tbl", sep="\t", quote=F, row.names=F)
write.table(Phenmat, file="annotated.Gohir.Phenmat.5pct.tbl", sep="\t", quote=F, row.names=F)
write.table(Seedmat, file="annotated.Gohir.Seedmat.5pct.tbl", sep="\t", quote=F, row.names=F)
write.table(PAmat, file="annotated.Gohir.PAmat.5pct.tbl", sep="\t", quote=F, row.names=F)
